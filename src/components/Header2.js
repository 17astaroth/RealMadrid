...
const Header2 = ({ className = "" }) => {
  ...
  return (
    <header
      className={`flex-1 flex flex-row items-start justify-center py-[9px] px-5 box-border sticky gap-[332px] top-[0] z-[99] max-w-full text-left text-xl text-black font-body-body-2 mq450:gap-[41px] mq1325:gap-[166px] mq800:gap-[83px] ${className}`}
    >
      ...
      <nav className="m-0 w-[248px] flex flex-col items-start justify-start pt-[17px] px-0 pb-0 box-border mq450:w-0 mq800:hidden">
        <nav className="m-0 self-stretch overflow-hidden flex flex-row items-start justify-between gap-[20px] z-[1] text-left text-sm text-text-gray-900 font-body-body-2 mq450:hidden">
          ...
          <div className="flex flex-row items-start justify-start">
            <a
              className="[text-decoration:none] relative leading-[20px] font-medium text-[inherit] inline-block min-w-[50px] cursor-pointer"
      ...
      <div className="flex flex-row items-start justify-start py-0 pr-[273px] pl-0 box-border gap-[16px] max-w-full mq800:pr-[136px] mq800:box-border">
